 ...
 * - Stores scroll position in a ref to avoid re-renders on every tick.
 * - Returns a state value that React renders; updated at most once per frame.
...
export function useWeightedScroll(): number {
  ...
  useEffect(() => {
    ...
    const measureVelocity = (): void => {
      ...
      if (lastTimeRef.current > 0) {
        ...
        if (dt > 0) {
          ...
          const instantVelocity = dy / dt; // px / ms
    ...
    const tick = (): void => {
      ...
      // Map velocity (px/ms) → font-weight. Higher velocity → thinner weight.
      // Typical scroll velocity is 0–3 px/ms.
      const v = Math.min(velocityRef.current, 3);
      const normalized = v / 3; // 0..1
      const weight = Math.round(MAX_WEIGHT - normalized * VELOCITY_GAIN * (MAX_WEIGHT - MIN_WEIGHT));
